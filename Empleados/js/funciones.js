function generarHtml(datos, div) {
    for (let i = 0; i < datos.length; i++) {
        div.innerHTML +=
            ...
            <div class='cards'>
                <hr>
                <input type='text' disabled class='empleado-input' name='idEmpleado' value="${datos[i].id}">
                ...
                <hr>
                <input type='image' data-toggle='modal' data-target='#actuModal' class='editbtn' src='https://th.bing.com/th/id/OIP.lRJfLfst1TmsMtg3pXJ1AAHaHa?pid=Api&rs=1' id="${datos[i].id}">
                <input type='image' data-toggle='modal' data-target='#deleteModal' class='delbtn' src='https://th.bing.com/th/id/OIP.90bvTUGcCJqR0qsvzA85HQHaHx?pid=Api&rs=1' id="${datos[i].id}">
        </div>
        </div>`;
    }
...
function validar(nombre, apellidos, telefono, correo, direccion) {
    if (nombre.length > 0 && apellidos.length > 0 && telefono.length > 0 && correo.length > 0 && correo.includes("@") && direccion.length > 0) {
        return true;
    } else {
